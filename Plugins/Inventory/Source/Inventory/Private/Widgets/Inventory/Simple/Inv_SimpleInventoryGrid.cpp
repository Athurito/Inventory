...
void UInv_SimpleInventoryGrid::NativeOnInitialized()
{
...
FInv_SlotAvailabilityResult UInv_SimpleInventoryGrid::HasRoomForItem(const UInv_ItemComponent* ItemComponent) const
{
	...
	// Allocate empty slots per stack / item
...
int32 UInv_SimpleInventoryGrid::FindFirstEmptySlotIndex() const
{
...
int32 UInv_SimpleInventoryGrid::FindExistingStackIndex(const FGameplayTag& ItemType, int32 MaxStackSize) const
{
	for (UInv_GridSlot* GridCell : GridSlots)
	{
		if (!IsValid(GridCell)) continue;
...
void UInv_SimpleInventoryGrid::PlaceItemInSlot(UInv_InventoryItem* Item, int32 SlotIndex, int32 StackAmount)
{
	...
	if (!SlottedItemClass || !CanvasPanel) return;
	...
	// Set the icon texture if available
	if (Icon)
	{
		SetSlottedItemImage(Slotted, Icon, FVector2D(TileSize, TileSize));
	}
}

void UInv_SimpleInventoryGrid::SetSlottedItemImage(const UInv_SlottedItem* SlottedItem, UTexture2D* Icon, const FVector2D& DrawSize) const
{
	FSlateBrush Brush;
...
void UInv_SimpleInventoryGrid::ConstructGrid()
{
	GridSlots.Reset();
